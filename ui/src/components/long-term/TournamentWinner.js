        ...
        {squads.map((team) => (
          <label
            ...
            className={`px-4 py-2.5 rounded-lg text-sm cursor-pointer border-2 transition-all select-none active:scale-95 ${
              selected === team.teamCode
                ? "bg-purple-600/30 border-purple-500 text-purple-200 shadow-lg shadow-purple-900/30"
                : "bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-500"
            } ${disabled ? "pointer-events-none opacity-70" : ""}`}
          >
            <input type="radio" name="tournament_winner" value={team.teamCode} checked={selected === team.teamCode} onChange={() => onSelect(team.teamCode)} disabled={disabled} className="sr-only" />
            {team.teamCode}
          </label>
        ))}
      </div>
