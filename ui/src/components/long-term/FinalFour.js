...
export default function FinalFour({ squads, selected, onToggle, points, disabled }) {
  return (
    <div className="card animate-slide-up" style={{ animationDelay: "100ms" }}>
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="font-semibold text-gray-100">Semi-Finalists (Final Four)</h3>
          <p className="text-xs text-gray-500">
            Pick 4 teams to reach the semi-finals
            {selected.length > 0 && <span className="text-purple-400 ml-2">{selected.length}/4</span>}
          </p>
        </div>
