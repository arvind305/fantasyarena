...
--
-- PROBLEM:
--   The old RPC used: total_score = leaderboard.total_score + new_match_score
--   This means re-scoring a match DOUBLES the points in the leaderboard.
--   Every re-score required manual fix-leaderboard.js + fix-ranks.js.
--
-- FIX:
--   Section 6 now RECALCULATES total_score from SUM(bets.score) for each user.
--   Re-scoring any match any number of times is now completely safe.
--   Ranks are recalculated automatically at the end.
--
-- SAFE TO RE-RUN: This is a CREATE OR REPLACE, applying it multiple times is fine.
...
BEGIN
    ...
    -- =========================================
    -- For each user who bet on this match, recalculate their TOTAL from ALL bets
    FOR v_bet IN
    ...
    LOOP
        DECLARE
            ...
            v_user_matches INT;
        BEGIN
            -- Recalculate from ALL scored bets for this user (not just this match)
            ...
            -- Upsert with CORRECT totals (recalculated, not incremented)
    ...
    -- Update leaderboard rankings (sets rank and previous_rank)
