...
--
-- PROBLEM:
--   The FOR loop iterating over player_picks JSONB with a BEGIN...EXCEPTION...END
--   subtransaction for UUID casting causes incorrect accumulation of points.
--   Only some slots are being scored, resulting in systematically low player_pick_points.
--   Affected matches: wc_m33 through wc_m39 (all scored with this buggy loop).
--
-- FIX:
--   Replace the PL/pgSQL FOR loop with a single aggregate SQL query using JOINs.
--   This avoids subtransaction/savepoint issues entirely and is also more efficient.
--
-- SAFE TO RE-RUN: This is CREATE OR REPLACE.
...
BEGIN
    ...
    LOOP
        ...
        -- =========================================
        -- 3. PLAYER PICKS SCORING (single aggregate query — replaces buggy FOR loop)
        -- =========================================
