...
--
-- Background: Two separate SQL files created overlapping versions:
--   1. 011_create_scoring_functions.sql -> (VARCHAR, VARCHAR) returning JSON
--   2. src/db/admin-scoring-function.sql -> (VARCHAR, VARCHAR, JSONB) returning JSONB
-- PostgreSQL stored one as (character varying, character varying) and the other
-- as (text, text), causing PostgREST error PGRST203.
--
-- Solution: Drop ALL overloads and recreate with TEXT params.
...
-- =====================================================
-- Step 2: Recreate with TEXT parameters (single version)
-- =====================================================
...
BEGIN
    ...
    LOOP
        ...
        -- display_name already found from leaderboard or stays NULL
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION calculate_match_scores(TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_match_scores(TEXT, TEXT) TO service_role;

COMMENT ON FUNCTION calculate_match_scores IS 'Calculate and apply scores for all bets on a match. Handles WINNER, TOTAL_RUNS, PLAYER_PICKS, SIDE_BETS, and RUNNERS. Should only be called by admin users.';
