...
-- This updates the calculate_match_scores function to look up display names
-- from the `users` table instead of the legacy `user_profiles` table.
-- The full function is re-created to apply the fix.
...
BEGIN
    ...
    -- FIX: Use `users` table instead of legacy `user_profiles`
    -- =========================================
    FOR v_bet IN
    ...
    LOOP
        ...
        IF v_user_display_name IS NULL THEN
            -- FIX: Fall back to `users` table (was `user_profiles` which doesn't exist)
            SELECT display_name INTO v_user_display_name
            FROM users
